...
//  Ising model for spin cross over nanochain

int main ()
{ Ran  ran(67); //initializing random number, pick any integer of you choice
 
  ofstream myfile;
  myfile.open("model2.dat");
  for ( double T=0.2; T<16; T=T+0.2 ) {
   ...
   while ( n < m ) {           // m  number of iteration for random sampling
    ...
    int  j = ran.doub()*dim ;
    int  k = ran.doub()*dim ;  // if the site is Al, skip calculation
   ...
   for ( int i=0; i<dim; i++) {
     for ( int j=0; j<dim; j++ ) {
       for ( int k=0; k<dim; k++ ) {
        sum3 = sum3 + arr[i][j][k] ; }}}
   double mg3 = abs ( ( (double)sum3 ) / ( dim*dim*dim ) ) ;
   double h;
     h = -1 + 1*T + 1*mg3  ;
   double energy;
 energy = u + h*arr[i][j][k] ;
      if ( energy <= 0 )   // if u<=0 flipping reduce energy
           arr[i][j][k] = -arr[i][j][k] ; 
      else if ( ran.doub() < exp( -energy/T) ) 
           arr[i][j][k] = -arr[i][j][k] ;
    ...
    for ( int i=0; i<dim; i++) {
       for ( int j=0; j<dim; j++ ) {
          for ( int k=0; k<dim; k++ ) {
           sum = sum + arr[i][j][k] ;  }}}
     double  mg  = abs ( ( (double)sum ) / ( dim*dim*dim ) )  ;
