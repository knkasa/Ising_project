...
//  Ising model of cubic lattice with external field

int main ()
{ Ran  ran(67); //initializing random number, pick any integer of you choice

 ofstream myfile;
  myfile.open("external.dat");
  for ( double T=0.2; T<8; T=T+0.1 ) {
  
    int h = 5;   //  external field
   ...
   while ( n < m ) {           // m  number of iteration for random sampling
    ...
    u = top + right + bottom + left + above + below +  h*arr[i][j][k]; 
       if ( u <= 0 ) 
   ...
   //  getting avg energy
     ...
     cout << "# of Al = " << a << endl;
     cout << "external field strength = " << h << endl;
